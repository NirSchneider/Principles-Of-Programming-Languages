...
import { applyEnv, makeExtEnv } from "../imp/L5-env";
...
// main purpose : Initialize the TEnv with all defined classes 
...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    let myEnv : E.TEnv =E.makeEmptyTEnv();
    let toAdd = R.map(A.classExpToClassTExp, A.parsedToClassExps(parsed));
    for (let i:number = 0 ; i< toAdd.length;i++){
        myEnv=E.makeExtendTEnv([toAdd[i].typename], [toAdd[i]] , myEnv)
    }
    return myEnv
}
...
//   (define (var : texp) val) (define)
    ...
    // If _Tenv |- _x1:_S1
    // Then _Tenv |- (define _x1:_S1 _e1)  : Void
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => { 
    let ExTenv:E.TEnv = E.makeExtendTEnv([exp.var.var],[exp.var.texp],tenv)
    bind(typeofExp(exp.val,ExTenv),(x:T.TExp)=>checkEqualType(x,exp.var.texp,exp))
...
function typeofProgramExps (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp>  {
    if (A.isDefineExp(exp) && isEmpty(exps)) {
        return typeofDefine(exp,tenv)
    }
    if (A.isDefineExp(exp)) {
        let extTEnv: E.TEnv= E.makeExtendTEnv([exp.var.var],[exp.var.texp],tenv)
        return bind(typeofDefine(exp,tenv),_=>typeofProgramExps(first(exps),rest(exps),extTEnv))
    }
    if (isEmpty(exps)){
...
export const typeofLit = (exp: A.LitExp): Result<T.TExp> =>
    //A.isCompoundExp(exp.val) ? makeOk(T.makePairTExp()) : makeOk(T.makeSymbolTExp(V.makeSymbolSExp()))
...
// Typing rule:
    ...
    // expressions _e1 and
    // type expressions _S1:
    // given : <Set-exp _x1 _e1>
    // If _Tenv |- _e1:_S1
    // If _Tenv |- _x1:_S1
    // Then _Tenv |- (set! _x1 _e1) : void

export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {// ********         TODO typeofSet       *********
    return safe2((typeOfVal:T.TExp ,typeOfVar:T.TExp)=> bind(checkEqualType(typeOfVal,typeOfVar,exp),(x:true)=>makeOk(T.makeVoidTExp())))
    (typeofExp(exp.val,tenv),E.applyTEnv(tenv,exp.var.var))
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    let myTEnv : E.TEnv = tenv;
    const typesOfFields:T.TExp[] = R.map((x:A.VarDecl)=> x.texp , exp.fields);
    const namesOfFields:string[] = R.map((y:A.VarDecl)=>y.var , exp.fields)
    for (let i : number =0; i<exp.fields.length; i++ ){
        myTEnv = E.makeExtendTEnv([namesOfFields[i]],[typesOfFields[i]],myTEnv);
    }
    
    const tVar:T.TExp[] = R.map((x:A.Binding)=> x.var.texp , exp.methods);
    const tVal = mapResult((x:A.Binding)=> typeofExp(x.val,myTEnv) , exp.methods);

    return safe2((v:T.TExp[],y:any)=>bind(checkEqualTypes(tVar,v,exp),(x)=>makeOk(T.makeProcTExp(R.map((varDecl)=>varDecl.texp,exp.fields),A.classExpToClassTExp(exp)))))
    (tVal,makeOk(R.identity))
}
