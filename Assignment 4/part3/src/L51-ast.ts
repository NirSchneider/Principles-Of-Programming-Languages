...
import { concat, chain, join, map, zipWith, identity } from "ramda";
...
const makeMethods = (methods :Sexp) : Result<Binding[]> => { 
    if (!isGoodBindings(methods)) {
        return makeFailure('Malformed bindings in "c" expression');
    }
    const vars = mapResult(parseVarDecl,map((b)=> b[0],methods)); // (func (lambda (a):T1 a+1))
    const vals = map((b)=> b[1],methods);
    const valsResult = mapResult(parseL5CExp,vals)
    return safe2((v:VarDecl[] , m :CExp[]) =>makeOk(zipWith(makeBinding, v , m )))(vars,valsResult) 
}
...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    if(isArray(varDecls) && isGoodBindings(bindings)){
        const vars = mapResult(parseVarDecl,varDecls); 
        return bind(makeMethods(bindings),(methods : Binding[])=>bind(vars,(fields:VarDecl[])=>makeOk(makeClassExp(makeTVar(typeName.toString()),fields,methods))))
    }
    return makeFailure("Expression not of the form ('class must have shape (class [: <type>]? <fields> <methods>)'")
}
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => 
    ...
    isAppExp (p) ? chain(parsedToClassExps, p.rands).concat(parsedToClassExps(p.rator)):
    isSetExp (p) ? chain(parsedToClassExps, [p.var,p.val]):
    isClassExp (p) ? [p].concat(chain(parsedToClassExps ,map((x:Binding)=> x.val,p.methods))):
    []
