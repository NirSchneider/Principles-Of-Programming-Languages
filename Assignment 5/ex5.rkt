...
(define append$
 (lambda (lst1 lst2 cont)
   (if (empty? lst1)
       (cont lst2)
       (append$ (cdr lst1) lst2 (lambda (return_from_lst1)
                                        (cont (cons (car lst1) return_from_lst1 )))))
  )
)
...
(define equal-trees$
  (lambda (tree1 tree2 succ fail)
    (cond ((and (empty? tree1) (empty? tree2))
           (succ tree1))
          ...
          ((and (leaf? (car tree1)) (leaf? (car tree2)))
           (equal-trees$ (cdr tree1)(cdr tree2) (lambda(res1)
                                                         (succ (cons (cons (car tree1) (car tree2)) res1))) fail))
          ((and (leaf? (car tree1)) (not (leaf? (car tree2))))
           (fail (cons (car tree1)(car tree2))))
          ((and (not (leaf? (car tree1))) (leaf? (car tree2)))
           (fail (cons (car tree1)(car tree2))))
          (else (equal-trees$ (car tree1) (car tree2) (lambda (res2)
                                                           (equal-trees$ (cdr tree1) (cdr tree2) (lambda (res3)
                                                                                                   (succ (cons res2 res3)))fail))fail))
          )
    )
  )                                                                  
...
;;integers-steps-from , reduce3-lzl ,map-lzl
;(+ (/ 1 (* a (+ a 2)))

(define generate-pi-approximations
  (lambda ()
    (map-lzl (lambda (y) (* y 8)) (reduce3-lzl + 0 (map-lzl (lambda (x) (/ 1 (* x (+ x 2))))(integers-steps-from 1 4))))
   )
)
