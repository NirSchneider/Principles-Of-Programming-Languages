...
const makeVarDeclR = (v:string):Result<VarDecl> =>
    makeOk (makeVarDecl(v));
        
const parseClassExp = (fie: Sexp ,met:Sexp) : Result <ClassExp> => 
    isArray(fie) && allT(isString, fie) ? 
    safe2((fields:VarDecl[] ,methods :Binding[])=> makeOk(makeClassExp(fields,methods)))
    (mapResult(makeVarDeclR,fie),makeMethods(met)):
    makeFailure("Expression not of the form (class ( <var>+ ) ( <binding>+ )")

const makeMethods = (met :Sexp) : Result<Binding[]> => {
    if (!isGoodBindings(met)) {
        return makeFailure('Malformed bindings in "c" expression');
    }
    const vars = map(b => b[0], met); // (func (lambda (a) a+1))
    const valsResult = mapResult((m)=> parseL31CExp(second(m)),met)
...
const unparseClassExp = (ce: ClassExp) : string => 
    `(class (${map((p: VarDecl) => p.var, ce.fields).join(" ")}) (${map((c: Binding) => `(${c.var.var} ${unparseL31(c.val)})`, ce.methods).join(" ")}))`
