...
import { any, filter, map, reduce, repeat, zipWith } from "ramda";
...
import { applyEnv, makeExtEnv, Env, Store, setStore, extendStore, ExtEnv, theGlobalEnv, globalEnvAddBinding, theStore,applyStore,applyEnvStore } from "./L21-env-store"; 
import { isClosure, makeClosure, Closure, Value } from "./L21-value-store";
import { applyPrimitive } from "./evalPrimitive-store";
import { first, rest, isEmpty } from "../shared/list";
import { Result, bind, safe2, mapResult, makeFailure, makeOk, either, isOk } from "../shared/result";
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses : number[] = map((x:Value) => unbox(extendStore(theStore, x).vals).length-1, args);
...
const evalDefineExps = (def: DefineExp, exps: Exp[]): Result<Value> => 
    either(bind(applicativeEval(def.val, theGlobalEnv), (a:Value) => makeOk(unbox(extendStore(theStore, a).vals).length-1)),
    (b:number) => (globalEnvAddBinding(def.var.var, b), evalSequence(exps, theGlobalEnv)), (c) => makeFailure("invalid expression"));
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        const addresses = map((x:Value) => unbox(extendStore(theStore, x).vals).length-1, vals)
...
const evalSet = (exp: SetExp, env: Env): Result<void> => 
    safe2((val: Value, bdg: number) => makeOk(setStore(theStore, bdg,val)))
